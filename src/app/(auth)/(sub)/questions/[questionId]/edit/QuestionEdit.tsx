...
export default function QuestionEdit({ question }: { question: any }) {
  ...
  return (
    <Formik
      ...
      onSubmit={async (values, { setSubmitting }) => {
        setSubmitting(true)
        const updatedQuestion = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/questions/${question.id}`, {
          method: 'POST',
          body: JSON.stringify({
            text: values.text,
                      ...
                      ? (
                        <div className="overflow-hidden rounded-md border border-gray-300 bg-white mt-3">
                          <ul role="list" className="divide-y divide-gray-300">
                            {
                              values.answers.map((answer: any, index: number) => (
