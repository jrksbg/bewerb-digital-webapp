...
export default async function StationenPage() {
  ...
  const stationen = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/bewerbstationen`, {
    method: 'GET',
    headers: {
    ...
    <div className="px-4 sm:px-6 lg:px-8">
      <div className="sm:flex sm:items-center">
        <div className="sm:flex-auto">
          <h1 className="text-base font-semibold text-gray-900">Stationen</h1>
          <p className="mt-2 text-sm text-gray-700">
            Stationen des JRK Bewerbs.
          </p>
        </div>
            ...
            <table className="min-w-full divide-y divide-gray-300">
              ...
              {stationen.map((station: any) => (
                <tr key={station.id}>
                  ...
                  <td className="px-3 py-4 text-sm whitespace-nowrap text-gray-500">{station.location}</td>
                  <td className="px-3 py-4 text-sm whitespace-nowrap text-gray-500">{mapStationType(station.type)}</td>
                  <td className="relative py-4 pr-4 pl-3 text-right text-sm font-medium whitespace-nowrap sm:pr-6 lg:pr-8">
                    <Link href={`/stationen/${station.id}`} className="text-rk-red-600 hover:text-rk-red-900">
                      Edit<span className="sr-only">, {station.name}</span>
                    </Link>
                  </td>
                </tr>
              ))}
